...
app.post("/generate-quiz", async (req, res) => {
  ...
  try {
    const { topic = "Aptitude", level = "medium" } = req.body;

    const prompt = `
Generate 10 aptitude MCQs for ${topic}.
...
`;
    ...
    res.json({
      blog: r?.choices?.[0]?.message?.content || "<p>No content</p>"
    });

  } catch (err) {
    console.log(err);
    res.json({ blog: "<p>Error</p>" });
  }
});
...
app.post("/ask-ai", async (req, res) => {
  ...
  try {
    // Example simple response (replace with OpenAI later)
    res.json({
